...
uv export --no-hashes --format requirements-txt \
  | python scripts/homebrew/generate_formula.py 0.2.11 \
...
class Jdkman < Formula
  ...
  desc "..."
  homepage "..."
  url "https://files.pythonhosted.org/.../jdkman-0.2.11.tar.gz"   # sdist
  sha256 "..."
  license "MIT"
  ...
  def install
    ...
    bin.install_symlink libexec/"bin/jdk_hook"
...
### sdist vs wheel 역할 분리

| 항목 | 사용 위치 | 이유 |
|---|---|---|
| `url` (sdist) | formula 상단 | Homebrew가 소스 기반 설치의 기준으로 사용 |
| `resource "jdkman-whl"` (wheel) | `def install` 안에서 pip로 설치 | sdist 빌드 없이 wheel을 직접 설치해 빌드 의존성 불필요 |

sdist는 Homebrew의 관례상 반드시 선언해야 하지만, 실제 설치는 wheel을 통해 이루어진다.
